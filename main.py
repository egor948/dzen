 ...
 **Найди 3-6 (НЕ БОЛЬШЕ 6!)** КАЧЕСТВЕННЫХ, АГРЕГИРОВАННЫХ сюжетов для статей. 
    **Ключевое требование:** Объединяй все смежные или похожие новости в один крупный сюжет.
    ...
    **ИНСТРУКЦИЯ ПО СОЗДАНИЮ `search_queries` (ПРИОРИТЕТЫ И ФОРМАТ):**
    Сначала выбери ОДНУ главную сущность сюжета, используя этот приоритет:
    1. **ПЕРСОНА** (Тренер, Игрок, Судья, Владелец). Если персон несколько, бери ПЕРВУЮ по важности.
    2. **КЛУБ** (Если персон нет).
    3. **КОНКРЕТНЫЙ МАТЧ** (Например, "Real Madrid vs Barcelona").
    4. **МЕСТО/СТАДИОН**.
    5. **СОРЕВНОВАНИЕ**.

    Теперь сформируй 3 запроса на основе выбранной сущности:
    *   **Первый запрос (Q1 - Основной):**
        *   Если Тренер/Судья/Владелец -> Просто Имя (Пример: "Hansi Flick").
        *   Если Футболист -> Имя + " football match" (Пример: "Lamine Yamal football match").
        *   Если Клуб/Матч/Стадион/Турнир -> Название + " football match" (Пример: "Barcelona football match", "El Clasico football match").
    *   **Второй запрос (Q2 - Упрощенный):** Такой же, как Q1, но если в Q1 было "football match", УБЕРИ ЕГО здесь. (Пример: Q1="Lamine Yamal football match" -> Q2="Lamine Yamal").
    *   **Третий запрос (Q3 - Резерв):** Максимально общее название сущности или темы (Пример: "FC Barcelona").
...
3.  **КЛЮЧЕВОЕ ПРАВИЛО:** Сосредоточься только на **ОДНОЙ, ГЛАВНОЙ ТЕМЕ** сюжета. **Избегай** включения в финальный текст мелких, бессвязных, нерелевантных или "мусорных" деталей, которые не развивают основную мысль статьи.
...
def write_summary_article(remaining_news, main_event_query):
    ...
    prompt = f"""**Роль:**
Ты — опытный контент-маркетолог и редактор Дзен-канала "НА БАНКЕ". Твоя задача — взять новостную сводку и превратить ее в "воздушный", динамичный и вирусный текст-дайджест.

**Главные правила:**
1.  **Пиши только на безупречном РУССКОМ языке.**
2.  **Основывайся СТРОГО на фактах из предоставленных новостей.**
3.  **УСТРАНЯЙ** формальные вступления (вроде "На этой неделе..." или "Мир большого спорта..."). Начинай СРАЗУ с самой горячей новости.
...
**2. Структура и Форматирование (используй HTML-теги):**
*   **Короткие абзацы:** Каждый абзац — это 1-3 коротких, ударных предложения.
*   **Подзаголовки-разделители:** Используй 2-3 интригующих подзаголовка, чтобы разбить дайджест на логические блоки (например, для каждой главной новости). Оберни их в теги `<b>` и `</b>`. **Пример:** `<b>Что происходит с «Ювентусом»?</b>`
*   **Выделение:** Выделяй ключевые мысли, имена и цифры жирным шрифтом. Используй теги `<b>` и `</b>`.

**3. Стиль:**
*   **"Крючок" в начале:** Первый абзац должен быть самым мощным. Начни с самой шокирующей или интригующей детали из всей сводки.
*   **Содержание:** **Сначала проанализируй НОВЫЙ ПОТОК НОВОСТЕЙ и выбери 3-5 САМЫХ ИНТЕРЕСНЫХ И СУЩЕСТВЕННЫХ.** Затем кратко, в 2-3 предложениях, расскажи о каждой из них.

**ЗАПРЕТЫ:**
*   **НИКОГДА** не используй формальные подзаголовки ("Введение", "Заключение", "Анализ").
*   **НИКОГДА** не используй Markdown (`#`, `*`). Только HTML-теги `<b>` и `</b>`.
*   **НИКОГДА** не пиши тексты рекламного, капперского или мошеннического характера.
...
async def find_real_photo_on_google(storyline):
    ...
    if not api_key or not cse_id:
        print("Основной ключ Google Search API не найден.")
        return None
