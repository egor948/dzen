...
1.  **Найди 3-6 (НЕ БОЛЬШЕ 6!)** КАЧЕСТВЕННЫХ, АГРЕГИРОВАННЫХ сюжетов для статей. 
    **Ключевое требование:** Объединяй все смежные или похожие новости в один крупный сюжет. Не создавай "тонких" сюжетов.
    ...
    Для каждого сюжета верни JSON-объект с полями: `title`, `category` и **строгий список `search_queries` из 2 элементов**:
    
    *   **Первый запрос (СТРОГИЙ):** Должен быть максимально **точным, контекстуальным и уникальным**. Включай имя персоны **и** название клуба/события. (Пример: "Hansi Flick Barcelona training" или "Vini Jr new contract signing").
    *   **Второй запрос (ОБЩИЙ, Резервный):** Должен быть **простым и широким** (только имя персоны или название клуба). Используется как запасной вариант. (Пример: "Hansi Flick" или "Barcelona team").
