...
def write_article_for_storyline(storyline):
    ...
    # ⬇️⬇️⬇️ НОВЫЙ, УЛУЧШЕННЫЙ ПРОМПТ ⬇️⬇️⬇️
    prompt = f"""[INST]Ты — первоклассный спортивный журналист и редактор, известный своим увлекательным и профессиональным стилем. Твоя задача — написать захватывающую статью для Яндекс.Дзен на основе предоставленных новостей.

**Рабочее название сюжета:** "{storyline['title']}"

**СТРОГИЕ ПРАВИЛА, КОТОРЫМ ТЫ ДОЛЖЕН СЛЕДОВАТЬ:**
1.  **ЗАГОЛОВОК:** Твой ответ должен начинаться с яркого, интригующего, но абсолютно правдивого заголовка. Заголовок должен быть на первой строке. НЕ ИСПОЛЬЗУЙ markdown (символы `*` или `**`) или слова "Заголовок:" в начале.
...
Твоя цель — создать готовый журналистский продукт, а не конспект.
[/INST]
...
def update_rss_file(processed_storylines):
    ...
    for storyline in reversed(processed_storylines):
        ...
        # ⬇️⬇️⬇️ Дополнительная очистка заголовка ⬇️⬇️⬇️
