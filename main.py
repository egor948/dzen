...
def write_article_for_storyline(storyline):
    ...
    prompt = prompt = f"""Ты — первоклассный спортивный журналист и редактор, пишущий для ведущего русскоязычного издания. Твоя задача — написать захватывающую, фактически точную и объемную статью на основе предоставленных новостей.
...
**ПРИМЕР ПЛОХОГО ОТВЕТА (ТАК ДЕЛАТЬ НЕЛЬЗЯ):**
"Breaking news: «Челси» совершили amazing трансфер. Их new player забил 五 голов."

**ПРИМЕР ХОРОШЕГО ОТВЕТА (ТАК ДЕЛАТЬ НУЖНО):**
"Срочные новости: «Челси» совершил потрясающий трансфер. Их новый игрок забил 5 голов."

**ТРЕБОВАНИЯ К ФОРМАТУ:**
1.  **Начинай сразу с заголовка.** Заголовок должен быть ярким, интригующим, но правдивым.
2.  **Пиши как эксперт:** глубокий анализ, увлекательный стиль, цельное повествование.
3.  **ЗАПРЕТЫ:** НИКОГДА не используй подзаголовки ("Введение", "Заключение"), дисклеймеры или маркеры ("Статья:").
"""
